...
public class RentalCompanyService extends AppUserService {
    ...
    public Long registerCompany(RentalCompany rentalCompany) {
        return repository.save(rentalCompany).getId();
    }

    public RentalCompany findRentalCompanyById(Long id){
        return (RentalCompany) repository.findById(id).orElseThrow(() -> new IllegalStateException("Company with ID= " + id + " not found"));
    }
